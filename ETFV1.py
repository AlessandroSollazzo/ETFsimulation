...
avg_err={}
avg_train_err={}
df = web.get_data_yahoo('SPY' ,start='2000-01-01',end='2019-02-01')
...
#There are some parameter that the machin learning can't learn over but needs to be iterated over. We use them to see which predefined functions or parameters yield the best fit function.
...
#Splitting the data into test and train sets. We passon the OHLC data with one day lag as the data frame X and the Close values of the current days as y.
#Let create a dictionary that holds the size of the train data set andits corresponding average prediction error.
#It's time the get the best-fit parameters to create a new function: we want to measure the performance of the regression function as compared to the size of the input database. For this reason we use the loop to iterate over the same data set but eith different lenghts.
#First we created a set of periodic numbers "t" starting from 50 to 97, in stps of 3. The purpose of thesenumbers is to choose the percentage size of the dataset that will be used as train data set.Second, for a given value of ‘t’ I split the length of the data set to the nearest integer corresponding to this percentage. Then I divided the total data into train data, which includes the data from the beginning till the split, and test data, which includes the data from the split till the end. The reason for adopting this approach and not using the random split is to maintain the continuity of the time series.
...
df= web.get_data_yahoo('SPY',start= '2000-01-01', end='2019-01-02')
